...
\begin{figure}[!t]
	\begin{center}
		\includegraphics[width=\textwidth, height=\textheight-75pt]{figures/product-tree-fw.drawio.pdf}
...
A list of the firmware tasks can be seen in the \autoref{tab:firmware-tasks}. A complete description of them is provided below.
...
This task deploys the Antenna module at the start of the mission. It basically implements the previously shown antenna deploy routine flowchart in \autoref{fig:flowchart_antenna_deploy_routine}.
...
\subsection{Data log} \label{sec:data-log-task}

This task saves the all subsystems last read housekeeping data to flash memory. Since there is no proper file system to manage the memories each subsystem got a predetermined number of flash pages to write in a ring buffer fashion. The address and page numbers for each subsystem can be seen in \textit{\href{https://github.com/spacelab-ufsc/obdh2/blob/master/firmware/config/config.h}{firmware/config/config.h}} from \cite{obdh2-repo}.

\subsection{General Telemetry}

The General Telemetry task transmits a data package containing the satellite's basic telemetry data every 60 seconds. The complete content of the packet can be seen at \autoref{tab:downlink-packets}. The transmission is controlled through a flag on the OBDH parameters (see \autoref{tab:vars-and-pars}), accessible through telecommands.

\subsection{EDC reading}

This task reads the state and housekeeping EDC frames, if there are available PTT packets on EDC's FIFO, it also attempts to read them. After every sucessfull PTT packet read the data is stored in flash memory, following the same scheme described in \autoref{sec:data-log-task}. A flowchart of how the data collection procedure works can be seen in \autoref{fig:edc-reading-flowchart}.

\begin{figure}[!ht]
	\begin{center}
		\includegraphics[width=\textwidth]{figures/edc_reading_flowchart.drawio.pdf}
		\caption{EDC reading procedure flowchart.}
		\label{fig:edc-reading-flowchart}
	\end{center}
\end{figure}
...
\begin{table}[!h]
	...
	\begin{tabular}{llc}
		...
		\bottomrule[1.5pt]
	\end{tabular}
	\caption{Mission Manager events.}
	\label{tab:mission-man-events}
\end{table}

\subsection{Payload X reading}

This task reads Payload X experiment data and status. It only attempts to read data from Payload X if it is active and set as the secondary payload (see \autoref{tab:vars-and-pars}).
...
This task processes all the telecommand packets received by the TTC device. The supported telemetry and telecommand packets, for both uplink and downlink, can be seen in \autoref{tab:packets-struct}. Also, a description of each telecommand is provided in \autoref{anx:packets}.
...
All telemetry data available to downloaded from OBDH is expected to be serialized in big endian ordering, meaning that a data field named ``param'' with type \textit{uint16}, is serialized as the first byte being the 8 most significant bits of ``param'' and the second byte is the 8 least significant bits of it. This behavior happens to all data types with length bigger than 1 byte.

The \autoref{anx:telemetry-apx} presents the available telemetry information for each subsystem, keep in mind that this only illustrates the timestamp and data fields of the complete downlink packet, there are still IDs, callsigns, etc. To see a complete representation of the packets look at the \autoref{tab:downlink-packets}.

\section{Telecommands} \label{sec:telecommands}

The \autoref{tab:packets-struct} summarizes all types of telemetry and telecommand packets received by TTC device that OBDH can handle, with the ID number, structure, length, and access type of each packet.
...
The ID of the subsystems, modules, memories and payloads used in the packets are highlighted in \autoref{tab:system-ids}.
...
\subsection{Authentication} \label{sec:hmac}
...
\subsection{Telecommand Descriptions}

A detailed description of each telecommand, including packet structure, execution and feedback is highlighted on \autoref{anx:packets}.
